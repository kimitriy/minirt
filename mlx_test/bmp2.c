...
int		get_pixel_color(t_all *all, int x, int y)
{
    char            *dst;
    unsigned int    color;
    dst = all->img.addr + (x * all->img.line_length + y * (all->img.bpp / 8));
...
int		screenshot(t_all *all)
{
    ...
    if ((generate_bitmap_image((unsigned char*)image, all->ray.screen_height, all->ray.screen_widght)) == 0)
        return (0);
...
int		ft_save(t_all *all)
{
    ...
    if ((all->pix.win = mlx_new_window(all->pix.mlx, all->map.x, all->map.y, "CUBED")) == 0)
        return (0);
