...
/*   Updated: 2021/03/19 20:09:03 by rburton          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_ambnt(t_scn *scn)
{
	printf("ambient: lvl:%f, trgb(%d, %d, %d)\n", scn->n_ambnt.lvl,
		scn->n_ambnt.trgb.r, scn->n_ambnt.trgb.g, scn->n_ambnt.trgb.b);
}
...
void	print_cam(t_scn *scn)
{
	...
	while (i < scn->n_cntr.cam)
	{
		...
		printf("cam: xyz(%f, %f, %f), nxyz(%f, %f, %f), fov:%d\n",
			...
			cam->v.nxyz.x, cam->v.nxyz.y, cam->v.nxyz.z, cam->fov);
...
void	print_light(t_scn *scn)
{
	...
	while (i < scn->n_cntr.lght)
	{
		...
		printf("lght: xyz(%f, %f, %f), lvl:%f, trgb(%d, %d, %d, %d)\n",
			lght->p.x, lght->p.y, lght->p.z, lght->lvl,
			lght->trgb.t, lght->trgb.r, lght->trgb.g, lght->trgb.b);
...
void	print_pln(t_scn *scn)
{
	...
	while (i < scn->n_cntr.pln)
	{
		...
		printf("pln: xyz(%f, %f, %f), nxyz(%f, %f, %f), trgb(%d, %d, %d, %d)\n",
			pln->p.x, pln->p.y, pln->p.z,
			pln->v.nxyz.x, pln->v.nxyz.y, pln->v.nxyz.z,
			pln->trgb.t, pln->trgb.r, pln->trgb.g, pln->trgb.b);
...
void	print_sphr(t_scn *scn)
{
	...
	while (i < scn->n_cntr.sphr)
	{
		...
		printf("sph: xyz(%f, %f, %f), d:%f, trgb(%d, %d, %d, %d)\n",
			sphr->p.x, sphr->p.y, sphr->p.z, sphr->d,
			sphr->trgb.t, sphr->trgb.r, sphr->trgb.g, sphr->trgb.b);
...
void	print_cyl(t_scn *scn)
{
	...
	while (i < scn->n_cntr.cyl)
	{
		...
		printf("cyl: xyz(%f, %f, %f), nxyz(%f, %f, %f), d:%f, h:%f, trgb(%d, %d, %d, %d)\n",
			cyl->p.x, cyl->p.y, cyl->p.z,
			cyl->v.nxyz.x, cyl->v.nxyz.y, cyl->v.nxyz.z,
			cyl->d, cyl->h, cyl->trgb.t, cyl->trgb.r, cyl->trgb.g, cyl->trgb.b);
...
void	print_sqr(t_scn *scn)
{
	...
	while (i < scn->n_cntr.sqr)
	{
		...
		printf("sqr: xyz(%f, %f, %f), nxyz(%f, %f, %f), side:%f, trgb(%d, %d, %d, %d)\n",
			...
			sqr->v.nxyz.x, sqr->v.nxyz.y, sqr->v.nxyz.z,
			sqr->side, sqr->trgb.t, sqr->trgb.r, sqr->trgb.g, sqr->trgb.b);
...
void	print_trngl(t_scn *scn)
{
	...
	while (i < scn->n_cntr.trngl)
	{
		...
		printf("trngl: xyz1(%f, %f, %f), xyz2(%f, %f, %f), xyz3(%f, %f, %f), trgb(%d, %d, %d, %d)\n",
			trngl->p1.x, trngl->p1.y, trngl->p1.z,
			trngl->p2.x, trngl->p2.y, trngl->p2.z,
			trngl->p3.x, trngl->p3.y, trngl->p3.z,
			trngl->trgb.t, trngl->trgb.r, trngl->trgb.g, trngl->trgb.b);
