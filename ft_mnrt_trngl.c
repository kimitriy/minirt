...
/*   Updated: 2021/02/19 23:56:16 by rburton          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	trgn_prmtr(t_trigon *trgn)
{
	...
	trgn->prmtr1 = (trgn->p_a.lngth + trgn->a_b.lngth + trgn->p_b.lngth) / 2;
	trgn->prmtr2 = (trgn->p_b.lngth + trgn->b_c.lngth + trgn->p_c.lngth) / 2;
	trgn->prmtr3 = (trgn->p_c.lngth + trgn->c_a.lngth + trgn->p_a.lngth) / 2;
}
...
void	trgn_area(t_trigon *trgn)
{
	...
	trgn->area1 = sqrtf(trgn->prmtr1 * (trgn->prmtr1 - trgn->p_a.lngth) * (trgn->prmtr1 - trgn->a_b.lngth) * (trgn->prmtr1 - trgn->p_b.lngth));
	trgn->area2 = sqrtf(trgn->prmtr2 * (trgn->prmtr2 - trgn->p_b.lngth) * (trgn->prmtr2 - trgn->b_c.lngth) * (trgn->prmtr2 - trgn->p_c.lngth));
	trgn->area3 = sqrtf(trgn->prmtr3 * (trgn->prmtr3 - trgn->p_c.lngth) * (trgn->prmtr3 - trgn->c_a.lngth) * (trgn->prmtr3 - trgn->p_a.lngth));
}
...
void	trgn_make(t_trngl *trngl, t_ray *ray)
{
	...
	{
		v_make(&trngl->trgn.p_a, &trngl->trgn.p, &trngl->trgn.a);
		v_make(&trngl->trgn.p_b, &trngl->trgn.p, &trngl->trgn.b);
		v_make(&trngl->trgn.p_c, &trngl->trgn.p, &trngl->trgn.c);
...
void	trngl_intrsct(t_scn *lscn, t_trngl *trngl, t_ray *ray)
{
	...
	if (fabsf(t) < INFINITY && t > 0.00006)
	{
		...
		v_fill(&o_p);
		p_calc(&trngl->trgn.p, &o_p, &ray->tail_p); //calculates p(x, y, z)
		trgn_make(trngl, ray);
	}
	if (trngl->trgn.p_in == '+' && t > 0 && t < ray->dist && ray->sgm == 0)
	{
		ray->dist = t;
		ray->obj = 't';
		ray->nrst = lscn->n_trngl;
		v_copy(&ray->vctr[0], &o_p);
		p_copy(&ray->hit_p, &trngl->trgn.p);
	}
	if (trngl->trgn.p_in == '+' && ray->sgm == 1 && t > 0 && ray->shdw != 'y' && t < ray->vctr[1].lngth)
		ray->shdw = 'y';
}
